...
sce$sample_id <- factor(sce$orig.ident.unique,
    levels = sce$orig.ident.unique %>% unique()
)
sce$cluster_id <- "ISC"
sce$group_id <- factor(sce$perturbation,
    levels = sce$perturbation %>% unique()
)
...
genes_of_interest <- c("DOR", "Atg8a", "EndoA", "Atg17", "Atg1")
...
pdf(here::here("scRNAseq", "plots", "main", "Fig8D_heatmap.pdf"),
