---
title: "Figure 7 sff expression in RNAi dataset"
...
progenitors$perturbation <- factor(progenitors$perturbation,
    levels = c("ctrl", "NotchRNAi", "NotchCphRNAi")
)

DefaultAssay(progenitors) <- "RNA"
Idents(progenitors) <- progenitors$perturbation

gene_of_interest <- "sff"
...
progenitor_pvals <- parallel::mclapply(comparisons, function(comp) {
  res <- FindMarkers(
    progenitors,
    ident.1 = comp[1],
    ident.2 = comp[2],
    ...
    mutate(comparison = paste0(comp[1], "_vs_", comp[2])) 
...
ggsave(
    plot = p,
    file = here("scRNAseq","plots", "main", "Fig7E_sff_expression_progenitors_RNAi.pdf"),
    width = 3.5,
    height = 3,
    units = "in",
    scale = 1.4
