...
```{r}
get_pvals_for_celltype <- function(seurat_obj,
                                   celltype,
                                   group1 = "notch", 
                                   group2 = "ctrl") {
  tmp <- seurat_obj[, seurat_obj$high_res_annotation == celltype]
...
isc_pvals <- get_pvals_for_celltype(progenitors, "ISC")
eep_pvals <- get_pvals_for_celltype(progenitors, "EEP")
...
ggsave(
    plot = p,
    file = here("scRNAseq","plots", "main", "Fig7D_sff_expression_isc_eep.pdf"),
    width = 3.5,
    height = 3,
    units = "in",
    scale = 1.4
