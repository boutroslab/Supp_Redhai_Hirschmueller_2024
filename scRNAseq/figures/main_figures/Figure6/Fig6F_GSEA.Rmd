...
hallmark <- gene_sets$hallmark %>%
    dplyr::select(term = pathway_name, genes = symbol)
...
gsea_NotchRNAi <- GSEA(ranked_NotchRNAi,
    TERM2GENE = rbind(hallmark, go_bp), 
    pvalueCutoff = 0.15
)
...
gsea_NotchCphRNAi <- GSEA(ranked_NotchCphRNAi,
    TERM2GENE = rbind(hallmark, go_bp), 
    pvalueCutoff = 0.15
)
...
ggsave(
    ...
    width = 10,
    height = 5,
    filename = here("scRNAseq","plots", "main","Fig6F_GSEA.pdf"),
    scale = 1
)
```
