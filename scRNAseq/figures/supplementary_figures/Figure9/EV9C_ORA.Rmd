...
```{r}
differentially_expressed_genes <- data.table::fread(here::here("output", "differential_expression", "pseudobulk_CphUp", "ISC", "edgeR_diff_expr.tsv"),
    data.table = F
)
...
for (direction in names(enrichr_res)) {
    ...
    GO_BP_filtered <- left_join(
        GO_BP_filtered,
        reducedTerms %>% dplyr::select(ID = go, parentTerm)
    ) %>% tidyr::drop_na()
    ...
    GO_BP_collapsed <- GO_BP_filtered %>%
        ...
        slice_max(., order_by=mean_zscore, n=20)
    ...
    ggsave(
        filename = here(
            "scRNAseq", "plots", "sup", str_interp("EV9C_ORA_ISC_${direction}.pdf")
        ),
