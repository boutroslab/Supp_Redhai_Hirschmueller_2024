...
seurat$perturbation <- factor(seurat$perturbation,
    levels = c("ctrl", "CphUp")
)
ISC <- seurat[, seurat$celltype_manual == "ISC"]
Idents(seurat) <- seurat$perturbation
res <- FindMarkers(
        seurat,
        ident.1 = "CphUp",
        ident.2 = "ctrl",
...
ggsave(
    plot = p,
    filename = here::here(
        "scRNAseq", "plots", "sup", "EV9A_Cph_expression_ISC.pdf"
    ),
    width = 2.9,
    height = 3,
    scale = 1.3
