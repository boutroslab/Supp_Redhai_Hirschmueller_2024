...
### Compare unperturbed progenitors to controls
...
# this contains no "missclassified" cells.
sce <- as.SingleCellExperiment(seurat[, seurat$VFC_2 == "ctrl_like"])
...
res_edgeR <- pbDS(pb, design = mm, method = "edgeR", contrast = contr, filter="samples")


# plot a volcano plot
library(ggtext)
plot_df <- res_edgeR$table$notch$progenitor %>% 
    mutate(significance = case_when(
        p_adj.loc<0.05 & logFC>0 ~ "up",
        p_adj.loc<0.05 & logFC<0 ~ "down",
        T ~ "not_sig")) %>% 
    mutate(log10pval = -log10(p_adj.loc))

p <- ggplot(plot_df, aes(x=logFC, y=log10pval, color=significance))+
    geom_point(shape=19, size=1, stroke=0.4)+
    theme_Publication()+
    ylab("-log<sub>10</sub> adj. P value")+
    xlab("log<sub>2</sub>FC")+
    theme(axis.title = element_markdown(size=15),
          legend.position = "none")+
    scale_y_continuous(limits = c(0,4), expand=expansion(mult=c(0.01,0.01)))+
    geom_hline(yintercept = -log10(0.05), linetype="dashed")+
    geom_vline(xintercept = 0, linetype="dashed")+
    scale_color_manual(values=c("#90bdff","#c6c6c6","#fba199"))
    

ggsave(plot=p,
       filename = here::here("plots","sup","volcano_unperturbed_vs_ctrl_progenitors.pdf"),
       width=2,
       height = 2,
       scale=2.6)
