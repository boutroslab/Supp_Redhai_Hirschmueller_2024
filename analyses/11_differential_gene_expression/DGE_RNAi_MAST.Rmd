...
```{r}
MAST_results <- readRDS(here("output", "followup_experiments", "differential_expression", "MAST", "MAST_res_coarse_batch.rds"))
...
for (celltype in names(MAST_results)) {
    for (condition in names(MAST_results[[celltype]])) {
        ...
        MAST_results[[celltype]][[condition]] %>%
            ...
            data.table::fwrite(.,
                file = here(
                    "output", "followup_experiments", "differential_expression",
