...
Here we are interested in running the comparision NotchRNAi vs NotchRNAi+CphRNAi.
...
### We are interested in the progenitor population
...
```{r}
MAST_results <- readRDS(here("output", "differential_expression", "MAST", "MAST_Progenitor_NotchRNAi_vs_NotchCphRNAi.rds"))
...
for (celltype in names(MAST_results)) {
    for (condition in names(MAST_results[[celltype]])) {
        dir.create(here("output", "differential_expression", "MAST", "NotchRNAi_vs_NotchCphRNAi", celltype),
        ...
        MAST_results[[celltype]][[condition]] %>%
            ...
            data.table::fwrite(.,
                file = here(
                    ...
                    "MAST", "NotchRNAi_vs_NotchCphRNAi", celltype,
                    str_interp("MAST_diff_expr_NotchRNAi_vs_NotchCphRNAi_${celltype}_coarse.tsv")
                ),
                sep = "\t",
                scipen = 50,
                row.names = F
...
### Now do this analysis for the ISC_EEP population
...
### Save the results in a human readable form
```{r}
MAST_results <- readRDS(here("output", "differential_expression", "MAST", "MAST_ISC_EEP_NotchRNAi_vs_NotchCphRNAi.rds"))
