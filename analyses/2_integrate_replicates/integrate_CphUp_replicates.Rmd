---
title: "Integration of the Cph overexpressing replicates"
...
## Introduction
After quality control (see 1_preprocessing) we integrate the 2 CphUp replicates using Seurat. 

```{r}
options(warn = 1)

# read in the QCed sc experiment (see 1_preprocessing for creation)
CphUp <- readRDS(here("output", "CphUp_qced.rds"))
...
file_names <- c("DS-0024B", "DS-00245")


CphUp_split <- lapply(setNames(seq_along(file_names),sample_ids), function(index){
  file <- file_names[index]
  sample <- sample_ids[index]
  print(sample)
  sce_tmp <- CphUp[[sample]]
  tmp <- Read10X(data.dir = here("raw_data", "CphUp", file, "raw_feature_bc_matrix"))
...
```{r}
features <- SelectIntegrationFeatures(object.list = CphUp_split)

# find anchors between experiments
anchors <- FindIntegrationAnchors(object.list = CphUp_split,
                                  anchor.features = features)

CphUp_integrated <- IntegrateData(anchorset = anchors, dims = 1:20)
DefaultAssay(CphUp_integrated) <- "integrated"
CphUp_integrated <- ScaleData(CphUp_integrated) %>% 
  RunPCA(., npcs = 50, verbose = F) %>% 
...
DefaultAssay(CphUp_integrated) <- "RNA"
saveRDS(CphUp_integrated, here("output", "CphUp_seurat_integrated.rds"))
```


# Make some diagnostic plots
```{r}
DimPlot(CphUp_integrated, group.by = "orig.ident")+
  small_axis("UMAP")+
  ggtitle("Integrated")
```
