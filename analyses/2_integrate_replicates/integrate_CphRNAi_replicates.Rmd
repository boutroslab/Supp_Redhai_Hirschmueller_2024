---
title: "Integration of the Cph RNAi replicates"
...
## Introduction
After quality control (see 1_preprocessing) we integrate the 2 CphRNAi replicates using Seurat. 

```{r}
options(warn = 1)

# read in the QCed sc experiment (see 1_preprocessing for creation)
CphRNAi <- readRDS(here("output", "CphRNAi_qced.rds"))
...
file_names <- c("DS-0024C", "DS-00246")


CphRNAi_split <- lapply(setNames(seq_along(file_names),sample_ids), function(index){
  file <- file_names[index]
  sample <- sample_ids[index]
  print(sample)
  sce_tmp <- CphRNAi[[sample]]
  tmp <- Read10X(data.dir = here("raw_data", "CphRNAi", file, "raw_feature_bc_matrix"))
  ...
  seurat_o <- CreateSeuratObject(counts = tmp, 
                                 project = sample, 
                                 min.cells = 0, 
                                 min.features = 0)
...
```{r}
features <- SelectIntegrationFeatures(object.list = CphRNAi_split)

# find anchors between experiments
anchors <- FindIntegrationAnchors(object.list = CphRNAi_split,
                                  anchor.features = features)

CphRNAi_integrated <- IntegrateData(anchorset = anchors, dims = 1:20)
DefaultAssay(CphRNAi_integrated) <- "integrated"
CphRNAi_integrated <- ScaleData(CphRNAi_integrated) %>% 
  RunPCA(., npcs = 50, verbose = F) %>% 
...
DefaultAssay(CphRNAi_integrated) <- "RNA"
saveRDS(CphRNAi_integrated, here("output", "CphRNAi_seurat_integrated.rds"))
```


# Make some diagnostic plots
```{r}
DimPlot(CphRNAi_integrated, group.by = "orig.ident")+
  small_axis("UMAP")+
  ggtitle("Integrated")
```
