---
title: "Integration of the Notch KO replicates"
...
## Introduction
After quality control (see 1_preprocessing) we integrate the 2 Notch replicates using Seurat. 

```{r}
options(warn = 1)

# read in the QCed sc experiment (see 1_preprocessing for creation)
notch <- readRDS(here("output", "notchKO_qced.rds"))
...
sample_ids <- c("TX22_N", "TX23_N")
file_names <- c("DS-000A4", "DS-000AC")


notch_split <- lapply(setNames(seq_along(file_names), sample_ids), function(index) {
    file <- file_names[index]
    sample <- sample_ids[index]

    sce_tmp <- notch[[sample]]
    tmp <- Read10X(data.dir = here("raw_data", "NotchKO", file, "raw_feature_bc_matrix"))
...
```{r}
features <- SelectIntegrationFeatures(object.list = notch_split)

# find anchors between experiments
anchors <- FindIntegrationAnchors(
    object.list = notch_split,
    anchor.features = features
)

notch_integrated <- IntegrateData(anchorset = anchors, dims = 1:20)
DefaultAssay(notch_integrated) <- "integrated"
notch_integrated <- ScaleData(notch_integrated) %>%
    RunPCA(., npcs = 50, verbose = F) %>%
...
DefaultAssay(notch_integrated) <- "RNA"
saveRDS(notch_integrated, here("output", "notchKO_seurat_integrated.rds"))
```
