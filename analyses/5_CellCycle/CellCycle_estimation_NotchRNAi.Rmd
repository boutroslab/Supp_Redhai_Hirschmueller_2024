---
title: "Estimate the Cell cycle for the notchRNAi data"
...
```{r}
notchRNAi <- readRDS(here("output", "notchRNAi_seurat_TX47_annotated_high_res.rds"))
...
res <- CellCycleScoring(notchRNAi,
    g2m.features = g2m_genes,
    s.features = s_genes
)

notchRNAi$S.Score <- res$S.Score
notchRNAi$G2M.Score <- res$G2M.Score
notchRNAi$Phase <- res$Phase
saveRDS(notchRNAi, here("output", "notchRNAi_seurat_TX47_annotated_high_res_cellcycle.rds"))
```

# Visualize scoring
```{r}
# check the distribution of phases per celltype
phase_distribution <- notchRNAi@meta.data %>%
    group_by(Phase, celltype_manual) %>%
    tally() %>%
    left_join(., table(notchRNAi$celltype_manual) %>%
