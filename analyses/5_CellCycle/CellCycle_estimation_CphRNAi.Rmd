...
stopifnot(all(Cells(CphRNAi) == lapply(CphRNAi_split, function(x) Cells(x)) %>%
    unlist() %>%
    unname()))

CphRNAi$S.Score <- lapply(CphRNAi_split, function(x) x$S.Score) %>%
    unlist() %>%
    unname()
CphRNAi$G2M.Score <- lapply(CphRNAi_split, function(x) x$G2M.Score) %>%
    unlist() %>%
    unname()
CphRNAi$Phase <- lapply(CphRNAi_split, function(x) x$Phase) %>%
    unlist() %>%
    unname()
