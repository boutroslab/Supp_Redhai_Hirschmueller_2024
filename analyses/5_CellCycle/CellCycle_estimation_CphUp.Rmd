...
stopifnot(all(Cells(CphUp) == lapply(CphUp_split, function(x) Cells(x)) %>% unlist() %>% unname()))

CphUp$S.Score <- lapply(CphUp_split, function(x) x$S.Score) %>% unlist() %>% unname()
CphUp$G2M.Score <- lapply(CphUp_split, function(x) x$G2M.Score) %>% unlist() %>% unname()
CphUp$Phase <- lapply(CphUp_split, function(x) x$Phase) %>% unlist() %>% unname()
