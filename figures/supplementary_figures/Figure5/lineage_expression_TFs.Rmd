...
trade_seq_diff <- readRDS(here("output", "tradeSeq_condition_test_res_strict.rds"))
diff_genes <- trade_seq_diff$Lineage4 %>%
    filter(padj_lineage4 < 0.05) %>%
    pull(gene)


# this list was downloaded from flymine
tf_list <- data.table::fread(here("raw_data", "flymine_results_2023-01-11T00-54-01.tsv"), header = F, data.table = F)

diff_tfs <- intersect(tf_list$V2, diff_genes)

# all except CG9650
diff_tfs <- diff_tfs[-which(diff_tfs == "CG9650")]
...
plots <- lapply(diff_tfs, plot_curves)
for (p in plots) {
    print(names(p$data)[4])
    ggsave(
        plot = p,
        width = 4,
        height = 2,
        filename = here("plots", "sup", "Fig5_lineage_expr", str_interp("Fig5_${names(p$data)[4]}_PCA_lineage_expr.pdf")),
        scale = 1.8
    )
}
```
