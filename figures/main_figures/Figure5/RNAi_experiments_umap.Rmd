...
special_dimplot <- function(seurat, feature, dim_reduction = "umap", ...) {
  ...
  centroids_data <- data %>% group_by(tmp_celltype) %>% summarise(m_umap1 = median(UMAP_1), 
...
p <- special_dimplot(seurat, feature = "tmp_celltype",size=0.05,
               shape=19,
               stroke=0.25)+
...
ggsave(
    plot=p,
    filename = here::here("plots", "main", "Fig5_RNAi_experiments_celltype.pdf"),
    width=5, 
    height=5
)
...
p <- new_dimplot(seurat, feature = "perturbation",size=0.05,
               shape=19,
               stroke=0.25)+
    small_axis("UMAP ", fontsize = 4, arrow_length = 17) +
...
ggsave(
    plot=p,
    filename = here::here("plots", "main", "Fig5_RNAi_experiments_condition.pdf"),
    width=5.5, 
    height=5
)
```
